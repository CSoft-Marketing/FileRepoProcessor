...
namespace FileRepoProcessor
{
    public class FileProcessor : IFileProcessor
    {
        ...
        public FileProcessor(ILogger<FileProcessor> logger, IPathMapper mapper, IOptions<PathOptions> options, IFileQueue queue)
        {
            _mapper = mapper;
            _logger = logger;
        ...
        //    // ✅ Create _p marker
        //    //CreateProcessingMarker(job.CacheFolder);
        //    // 🔥 Try to acquire lock
        ...
        //        //job.OCID = await StartOC(job.FilePath, job.CacheFolder);
        ...
        //            return;
        //        }
        //        // Non-fetchable CAD → done after OC submission
        ...
        private void ProcessWord(string filepath, string cacheFolder)
        {
            try
            {
                ...
                var fnameonly = System.IO.Path.GetFileNameWithoutExtension(filepath);
                var fname = $"{fnameonly}.pdf";
                var cacheFPath = System.IO.Path.Combine(cacheFolder, fname);
                doc.SaveToFile(cacheFPath, Spire.Doc.FileFormat.PDF);
                _logger.LogInfo("Saved portable version in cache");
                ProcessPDFForOtherFormats(cacheFPath, cacheFolder, filepath);
                _logger.LogInfo("Converting portable version in cache...");
                XorFileStream(cacheFPath, System.IO.Path.Combine(cacheFolder, "convert.dat"),0xAA);
                System.IO.File.Delete(cacheFPath);
                /*
        ...
        public static void PrepareWorksheetForPdf(Worksheet worksheet)
        {
            ...
            worksheet.Range[$"A1:{endCell}"].AutoFitRows();
            ...
            //worksheet.PageSetup.Zoom = 0; // enable FitToPages
        ...
        private void ProcessExcel(string filepath, string cacheFolder)    
        {
            try
            {
                ...
                ToPdfParameterList pdfParams = new ToPdfParameterList();
                ///////////////////For reference Keep it if you want
                ////1 page pdf per sheet containin everything/////////////
                ...
                //    // Optional but useful
                //    worksheet.PageSetup.IsPrintGridlines = true;   // show grid
                //    worksheet.PageSetup.IsPrintHeadings = true;    // A,B,C / 1,2,3 (if supported)
                //}
                foreach (Worksheet worksheet in workbook.Worksheets)
                {
                ...
                workbook.SaveToFile(cacheFPath, Spire.Xls.FileFormat.PDF);
                _logger.LogInfo("Saved pdf version in cache");
                ProcessPDFForOtherFormats(cacheFPath, cacheFolder, filepath);
        ...
        private void ProcessPowerPoint(string filepath, string cacheFolder)
        {
            try
            {
                ...
                ppt.SaveToFile(cacheFPath, Spire.Presentation.FileFormat.PDF);
                _logger.LogInfo("Saved pdf version in cache");
                ProcessPDFForOtherFormats(cacheFPath, cacheFolder, filepath);
                _logger.LogInfo("Converting portable version in cache...");
                XorFileStream(cacheFPath, System.IO.Path.Combine(cacheFolder, "convert.dat"), 0xAA);
        ...
        private void ProcessPDF(string filepath, string cacheFolder)
        {
            try
            {
                ...
                string TPath = System.IO.Path.Combine(cacheFolder, "_T");
                saveTFile(filepath, TPath, count);
                //save the obfuscated file in cache as dat file
                var datPath = System.IO.Path.Combine(cacheFolder, $"convert.dat");
                XorFileStream(filepath, datPath, 0xAA);
                pdfDocument.Dispose();
            }
            catch (Exception ex)
            {
