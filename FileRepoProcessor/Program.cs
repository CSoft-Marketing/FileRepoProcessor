...
IHost host = builder
    ...
    .ConfigureServices((context, services) =>
    {
        ...
        string osSpecificFolder;
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            osSpecificFolder = "plugin_linux";
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            osSpecificFolder = "plugin_mac";
        else
            osSpecificFolder = "plugin_windows";

        string pluginsPath = Path.Combine(AppContext.BaseDirectory, osSpecificFolder, "plugins");

        HtmlConverter.PluginPath = pluginsPath;
        ///////////////
        var browserInfo = BrowserLocator.Locate();

        Log.Information("======================================");
        Log.Information("HTML Rendering Engine Detection");
        Log.Information("======================================");

        if (browserInfo.HasChromiumBrowser)
        {
            Log.Information("Browser     : {Browser}", browserInfo.Type);
            Log.Information("Version     : {Version}", browserInfo.Version);
            Log.Information("Executable  : {Path}", browserInfo.ExecutablePath);
        }
        else
        {
            Log.Information("No Chromium browser detected.");
            Log.Information("Qt renderer will be used.");
        }
        ...
        services.AddSingleton(browserInfo);
